...
// useMemo
// useCallback
// padding element as children and regular props (children  do not re-render when parent get re-render)
...
// 3. code called bundle and this one hug bundle has to be download client side before start application it is done only initial load
// 4. hence then user go another page the browser did not request again because browser has already all code to execute on it's client side
// 5. hence application did not reload hence application became a single page application 😮 DIDN'T YOU KNOW😄
// 3. but downloading a hug bundle it take long time to download all javascript files at once so rather than making hug
// 4. bundle code splitting tachnique simply split hug file or bundle into multple javascript file so that as per user need each
